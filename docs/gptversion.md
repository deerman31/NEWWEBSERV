...
まず、「サーバーがブロックしてはならない」という部分は、サーバーがクライアントからのリクエストを処理している間、他のリクエストの処理を停止しないことを意味します。これは、サーバーがマルチスレッドまたは非同期処理を行うことで、複数のクライアントからのリクエストを同時に、効率的に処理できるようにすることを目的としています。
